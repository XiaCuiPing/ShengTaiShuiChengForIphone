...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        _checkBox.image = [NSBundle imageWithBundleName:@"DSXUI" imageNamed:@"imagepicker-checkbox.png"];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    _viewController.navigationItem.leftBarButtonItem = [DSXUI barButtonWithStyle:DSXBarButtonStyleBack target:self action:@selector(closeWin)];
    ...
    if (imageObject.isChecked) {
        ...
        if (_maxSize) {
            if (_maxSize > [_selectedAssets count]) {
                ...
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@""
                                                                    message:[NSString stringWithFormat:@"一次最多只能选择%ld张照片", (long)_maxSize]
                                                                   delegate:nil
                                                          cancelButtonTitle:@"取消"
                                                          otherButtonTitles:nil];
                [alertView show];
